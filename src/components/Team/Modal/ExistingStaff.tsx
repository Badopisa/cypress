...
import {addSelectedStaffsToTeam, checkSelectedStaff, getAllStaffs} from "@/store/actions/staffActions";
...
const ExistingStaff = ({
    ...
    const {
        filteredStaffs,
        currentTeam
    }: { filteredStaffs: any, currentTeam: any } = useSelector((state: RootStateOrAny) => state.team)
    const {selectedStaffs}: { selectedStaffs: any } = useSelector((state: RootStateOrAny) => state.staff)
    ...
    const handleSelect = () => {
        dispatch(addSelectedStaffsToTeam(selectedStaffs, currentTeam.id, user?.clubs[0]?.id, toast, onClose, setSelectedConfirmation))
    };
